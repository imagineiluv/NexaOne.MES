...
public sealed class IvtModuleCompositionTests
{
    ...
    public void Module_builds_one_shared_instance_for_each_public_export()
    {
        ...
        module.GetStockBillingDirectory().Should().BeAssignableTo<IStockBillingDirectory>();
        module.GetStockBillingDirectory().Should().BeSameAs(module.GetStockBillingDirectory());
        module.GetStockMasterExportWorker().Should().BeAssignableTo<IHostedService>();
    ...
    public void Spring_xml_boots_the_module_and_resolves_only_its_public_exports()
    {
        ...
        factory.GetObjectsOfType(typeof(IHostedService)).Keys
            .Should().BeEquivalentTo(["stockMasterExportWorker", "traceMaterialConsumptionWorker"]);
    }
