...
/// <summary>CMMS 얇은 브리지 컨트롤러 HTTP 매핑 검증(ADR-008, S5) — modules OFF + 가짜 ICmmsBridge 주입으로
/// 권한(403/200)·생성(200·검증 400)·상태전이(204·Conflict→409·NotFound→404)를 Spring/ALC 없이 결정적으로 검증한다.
/// 작업지시·보전계획·예비품 3개 애그리거트의 쓰기 경로(cmms:manage 게이트)를 커버한다.</summary>
public sealed class CmmsBridgeControllerTests : IClassFixture<CmmsBridgeControllerTests.BridgeFactory>
{
    private const string Secret = "phase-cmms-bridge-e2e-jwt-secret-key-at-least-32b";
    private const string Issuer = "nexaone-cmms-bridge-test";
    private readonly BridgeFactory _factory;
    public CmmsBridgeControllerTests(BridgeFactory factory) => _factory = factory;

    public sealed class BridgeFactory : WebApplicationFactory<Program>
    {
        public readonly string DbPath = Path.Combine(Path.GetTempPath(), $"nexaone-cmms-bridge-{Guid.NewGuid():N}.db");
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureTestServices(s => s.AddSingleton<ICmmsBridge>(new FakeBridge()));
        }
    ...
    private sealed class FakeBridge : ICmmsBridge
    {
    ...
    private HttpClient Client(params string[] permissions)
    {
        ...
        var claims = new List<Claim> { new(ClaimTypes.NameIdentifier, "cmms-bridge-tester") };
    ...
    [Fact]
    public async Task CreateWorkOrder_without_cmms_manage_is_forbidden()
    {
        var res = await Client("fdc:read").PostAsJsonAsync("/api/v1/cmms/work-orders",
            new { woId = "WO1", equipmentId = "EQ1", woType = "PM", description = "점검", assigneeId = "U1" });
        res.StatusCode.Should().Be(HttpStatusCode.Forbidden, "cmms:manage 미보유 쓰기는 403");
    }

    [Fact]
    public async Task AdjustStock_without_cmms_manage_is_forbidden()
    {
        var res = await Client().PostAsJsonAsync("/api/v1/cmms/spare-parts/P1/adjust-stock", new { delta = 5m });
    ...
    [Fact]
    public async Task CreateWorkOrder_with_cmms_manage_returns_200_with_dto()
    {
        var res = await Client("cmms:manage").PostAsJsonAsync("/api/v1/cmms/work-orders",
    ...
    {
        var res = await Client("cmms:manage").PostAsync("/api/v1/cmms/work-orders/WO1/start", content: null);
    ...
    {
        var res = await Client("cmms:manage").PostAsJsonAsync("/api/v1/cmms/work-orders/CONFLICT/complete",
            new { remark = "완료" });
    ...
    {
        var res = await Client("cmms:manage").PostAsync("/api/v1/cmms/work-orders/MISSING/cancel", content: null);
    ...
    [Fact]
    public async Task CreatePlan_with_cmms_manage_returns_200_with_dto()
    {
        var res = await Client("cmms:manage").PostAsJsonAsync("/api/v1/cmms/plans",
    ...
    {
        var res = await Client("cmms:manage").PostAsync("/api/v1/cmms/plans/CONFLICT/start", content: null);
    ...
    [Fact]
    public async Task CreatePart_with_cmms_manage_returns_200_with_dto()
    {
        var res = await Client("cmms:manage").PostAsJsonAsync("/api/v1/cmms/spare-parts",
    ...
    {
        var res = await Client("cmms:manage").PostAsJsonAsync("/api/v1/cmms/spare-parts/SP1/adjust-stock", new { delta = -3m });
    ...
    {
        var res = await Client("cmms:manage").PostAsJsonAsync("/api/v1/cmms/spare-parts/MISSING/adjust-stock", new { delta = 1m });
    ...
    {
        var res = await Client("cmms:manage").PostAsJsonAsync("/api/v1/cmms/spare-parts/SP1/adjust-stock", new { delta = -5000m });
