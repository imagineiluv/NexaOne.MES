...
public sealed class BillingWorkflowResourcesTests
{
    ...
    internal static Dictionary<string, string> ExpectedResources()
    {
        ...
        foreach (var file in new[] { "BillingWorkflowPanel.razor", "BillingWorkflowPanel.razor.cs", "HostBillingWorkspace.razor", "RecurringWorkflowPanel.razor", "RecurringWorkflowPanel.razor.cs", "HostRecurringWorkspace.razor", "HostFinancialReportHistory.razor", "HostExpenseWorkspace.razor", "HostExpenseWorkspace.razor.cs" })
        {
            ...
            Regex.Matches(source, """\bT\(\s*"(?:billing|inventory|recurring|reportHistory|expenseWorkspace|crm)\.[^"\\]+["]""").Count.Should().Be(calls.Count,
                "all ERP workspace calls must have literal fallbacks covered by this contract in {0}", file);
            foreach (Match call in calls)
            {
                var key = call.Groups["key"].Value;
                (key.StartsWith("billing.", StringComparison.Ordinal) || key.StartsWith("inventory.", StringComparison.Ordinal) || key.StartsWith("recurring.", StringComparison.Ordinal) || key.StartsWith("reportHistory.", StringComparison.Ordinal) || key.StartsWith("expenseWorkspace.", StringComparison.Ordinal) || key.StartsWith("crm.", StringComparison.Ordinal)).Should().BeTrue(key);
        ...
        expected.Keys.Count(key => key.StartsWith("expenseWorkspace.", StringComparison.Ordinal)).Should().BeGreaterThan(50);
        return expected;
    }
