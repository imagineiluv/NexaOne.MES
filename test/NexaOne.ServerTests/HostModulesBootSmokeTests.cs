...
/// 기동해 13개 모듈 + 워커 + 선언형 catalog의 전체 Bridge가 한 프로세스에서 실제로 올라오는지 검증한다.
...
public sealed class HostModulesBootSmokeTests
{
    ...
    public async Task Host_boots_all_fifteen_modules_workers_and_bridges_in_one_process()
    {
        ...
        root.GetProperty("services").GetArrayLength().Should().Be(15,
            "15개 도메인 모듈(Mdm·Est·Fdc·Ivt·Rms·Qms·Ems·Pom·Prc·Shp·Hr·Erp·Crm·Sls·Sys)이 모두 로드돼야 한다 — "
