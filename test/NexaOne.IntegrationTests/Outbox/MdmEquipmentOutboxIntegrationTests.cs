...
/// </summary>
public sealed class MdmEquipmentOutboxIntegrationTests
    : IClassFixture<TestApiFactory>, IClassFixture<OutboxEnabledTestApiFactory>
{
    ...
    public async Task Deactivate_writes_outbox_in_same_transaction_when_enabled()
    {
        ...
        OutboxCount(_on.ConnectionString, "EQ-OB-ON", "EquipmentDeactivated").Should().Be(1,
    ...
    public async Task UpdateInfo_writes_no_outbox_even_when_enabled()
    {
        ...
        using var conn = new SqliteConnection(_on.ConnectionString);
        conn.Open();
        using var cmd = conn.CreateCommand();
    ...
    public async Task Deactivate_does_not_write_outbox_when_disabled()
    {
        ...
        OutboxCount(_off.ConnectionString, "EQ-OB-OFF", "EquipmentDeactivated").Should().Be(0,
