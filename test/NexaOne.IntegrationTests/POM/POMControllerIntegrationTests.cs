...
///
/// 참고(orders 미검증): POM_PRODUCTION_ORDER 테이블을 생성하는 마이그레이션이 db/migrations 에 존재하지 않아
///   /api/v1/pom/orders 계열 엔드포인트는 SQLite 에서 'no such table' 로 실패한다(suspectedBugs 참조).
///   본 테스트는 실재하는 plans 경로만 검증한다.
/// </summary>
