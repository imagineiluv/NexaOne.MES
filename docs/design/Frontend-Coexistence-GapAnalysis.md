...
| **1** 엔진 승격 | ✅ Query/Security 완료, Event Bus 토대 | Query Gateway(`IQueryGateway`, 읽기 `/query`+쓰기 `/command` 명명쿼리, 권한 fail-fast)·Security PEP(permission 정책 전 컨트롤러 적용)는 완료. **Event Bus는 토대(opt-in)** — outbox 적재 + `IMessageBus` 추상화 + 디스패처/Consumer 글루는 있으나 `Kafka:Enabled`·`Events:Outbox:Enabled` 기본 false라 **기본 비기동**(appsettings에 키 명시). **브로커리스 인메모리 버스 추가**(2026-06-14): `InMemoryMessageBus`+`InMemoryBusSubscriberService`로 `Events:Outbox:Enabled && !Kafka:Enabled` 시 Kafka 없이 outbox→발행→SignalR 구독을 인프로세스로 동작(dev/단일노드, 단위테스트로 검증). **도메인이벤트→outbox 트랜잭션 통합 대표 슬라이스**(2026-06-14): EST 설비 상태 변경이 `EquipmentStateChangedDomainEvent`(IOutboxEvent)를 발행하고, EquipmentStateRepository가 상태·이력·이벤트를 **동일 트랜잭션**(ExecuteManyAsync)에 기록한다(opt-in `Events:Outbox:Enabled`, 기본 off — 적체 없음). EES_OUTBOX SQLite 자동증가 INSERT는 프로브로 검증(부트스트랩 수정 불요). **알람 애그리거트로 확장**(2026-06-14): EST 알람 발생/해제가 `EquipmentAlarmRaised`/`EquipmentAlarmCleared`(IOutboxEvent)를 발행하고 EquipmentAlarmRepository가 알람 행·이벤트를 동일 트랜잭션에 기록(opt-in, 동일 게이트). outbox INSERT 매핑은 `OutboxStatements` 헬퍼로 추출해 상태·알람 두 리포가 공유(중복 제거 — 확산 시 리포당 1줄). **전 lifecycle 애그리거트 확산 완료**(2026-06-15): `OutboxStatements`를 공유 `NexaOne.Infrastructure.Persistence`로 승격하고, 13개 상태전이 애그리거트(EST 2 + POM Lot/ProductionOrder/ProductionPlan, QMS Defect, MDM Equipment, CMMS WorkOrder/MaintenancePlan, RMS Recipe, SHP DeliveryOrder, SYS UserRequest/User, FDC AlarmHistory/InterlockHistory)에 동일 패턴 적용(opt-in, 기본 off). 마스터데이터 CRUD·append-only 로그는 이벤트 노이즈라 스캔 후 제외. 읽기경로 안전: ProductionOrder·MaintenancePlan·FDC 2개 history의 ToDomain이 Create+전이재생이라 Restore 팩토리로 교정(phantom 이벤트 방지, [[project_readpath_restore_pattern]]). **계정잠금 outbox화**(2026-06-15): SYS.User 잠금 전이는 동시성 안전을 위해 원자 SQL UPDATE가 소유하므로, 리포가 같은 트랜잭션에 조건부 INSERT…SELECT로 `UserAccountLocked`를 전이 시 1건만 기록(이미잠김 재시도 미중복). **로컬 Kafka/Redis는 `docker-compose.dev.yml`로 기동**(opt-in; 인메모리 버스는 브로커 불요). 검증: 단위 1008·통합 253/1skip. **실시간 알림 버스 일원화 완료**(2026-06-15): 구독자(인메모리+Kafka 공용 `RealtimeNotificationDispatch`)를 이벤트 유형별 세분 알림으로 보강하고, 컨트롤러는 `RealtimeNotificationCoordinator`로 버스 활성 시 '이벤트로 뒷받침되는' 직접호출을 생략(이중 발행 방지)·비활성 시 즉시 직접 발행(폴백). **브로커리스 버스를 기본 활성**(appsettings `Events:Outbox:Enabled=true`)으로 전환. 대응 이벤트가 없는 알림(고빈도 FDC 수집·인터록 발동)은 항상 직접 발행. 트레이드오프: 버스 경유 알림은 디스패처 폴링(`PollIntervalMs`=5s)만큼 지연 가능 — 필요 시 주기 하향 또는 `Events:Outbox:Enabled=false`(무지연 폴백). 통합 253/1skip·단위 1008 유지. 잔여(결정 필요): 없음(SPA Blazor 임베드는 Phase 2 참조) |
| **2** Pro-Code | ✅ 토대 | React+Vite+TS SPA(`src/01.Web/NexaOne.Spa`)+NSwag+permission 가드+토큰갱신. **빌드 E2E 검증 완료**(2026-06-15): SQLite로 API 기동 → `gen:api`(NSwag 클라이언트 생성) → `tsc -b` → `vite build`로 `dist` 산출 성공. 스캐폴딩 빌드버그 2건 교정(`@types/node` 추가, `tsconfig.node.json` composite emit). 생성 클라이언트·dist는 gitignore(빌드 산출물, 재생성). **Blazor 셸 임베드 완료**(2026-06-15): vite `base=/spa/`+`outDir→NexaOne.Web/wwwroot/spa`, NexaOne.Web이 `/spa`로 정적 서빙 + SPA 폴백(`MapFallbackToFile`). 런타임 검증: `/spa/index.html` 200·자산 `/spa/` 기준·클라이언트 라우트 폴백 200. 임베드 산출물은 gitignore(`npm run build`로 재생성, Blazor publish 시 함께 배포). 잔여: 없음 |
