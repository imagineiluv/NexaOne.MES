...
| **2** Pro-Code | ✅ 토대 | React+Vite+TS SPA(`src/01.Web/NexaOne.Spa`)+NSwag+permission 가드+토큰갱신. **빌드 E2E 검증 완료**(2026-06-15): SQLite로 API 기동 → `gen:api`(NSwag 클라이언트 생성) → `tsc -b` → `vite build`로 `dist` 산출 성공. 스캐폴딩 빌드버그 2건 교정(`@types/node` 추가, `tsconfig.node.json` composite emit). 생성 클라이언트·dist는 gitignore(빌드 산출물, 재생성). 잔여: Blazor 셸 임베드(제품 결정) |
