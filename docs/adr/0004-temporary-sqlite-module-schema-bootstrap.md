...
증분 복구한다. 이번 변경의 FDC effect lifecycle과 IVT TRACE cursor는 열 추가뿐 아니라 기존 행 backfill,
canonical trigger 교체, durable reconciliation marker를 한 `BEGIN IMMEDIATE` 경계에서 처리해야 한다.
...
- target: `FDC_INTERLOCK_HISTORY`의 V146 lifecycle evidence와
...
3. FDC·IVT가 위 객체의 backfill·trigger·marker를 소유하고 Common은 업무 테이블명을 알지 않는다.
4. 기존 pre-V142/pre-V146 DB upgrade와 crash 재기동 회귀가 그대로 통과한다.
