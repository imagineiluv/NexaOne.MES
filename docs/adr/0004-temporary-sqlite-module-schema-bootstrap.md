...
증분 복구한다. 이번 변경의 FDC effect lifecycle·GLOBAL runtime ownership fence와 IVT TRACE cursor·material configuration은
...
`INSERT OR REPLACE` guard·invalid timestamp partial index와
  ...
  `IVT_TRACE_CONSUMPTION_BINDING`·`IVT_MATERIAL_FEED_SESSION`·`IVT_MATERIAL_LOT`·
  `IVT_MATERIAL_CONSUMPTION_HISTORY`·`IVT_TRACE_BINDING_COMMAND`·`IVT_FEED_SESSION_COMMAND`의
  V151 source identity/LOT reservation/typed feed-session reference/command-ledger reconciliation과
  SQLite FK 동등 parent·key·delete guard
...
4. 기존 pre-V142/pre-V146/pre-V149/pre-V150/pre-V151 DB upgrade와 crash 재기동 회귀가 그대로 통과한다.
