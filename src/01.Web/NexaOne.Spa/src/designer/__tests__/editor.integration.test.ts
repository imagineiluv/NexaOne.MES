...
const CATALOG: QueryCatalog = {
  reads: ['MDM.PlantList'],
  writes: [{ id: 'SYS.Save', requiredPermission: 'sys:manage' }],
}
...
// ScreenEditor.tsx init 흐름 재현: init → 8 type addType → 8 block BlockManager.add.
...
describe('GrapesJS 실 에디터 통합(마운트)', () => {
  ...
  it('2. BLOCK_DEFS 8개 블록 id가 실 BlockManager에 모두 등록된다', () => {
    const ed = mountEditor()
    const ids = ed.BlockManager.getAll().map((b: { id: string }) => b.id).sort()
    expect(ids).toEqual(BLOCK_DEFS.map(b => b.id).sort())
  })

  it('3. COMPONENT_TYPE_DEFS 8개 type이 실 DomComponents에 resolve된다', () => {
    const ed = mountEditor()
    for (const c of COMPONENT_TYPE_DEFS) {
          ...
          children: [
            ...
            },
            {
              ...
              children: [
                ...
                { kind: 'commandButton', label: '저장', command: 'SYS.Save' },
              ],
            },
          ],
        },
