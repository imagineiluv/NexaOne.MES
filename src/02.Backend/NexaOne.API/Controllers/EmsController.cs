...
[Route("api/v1/ems")]
[Authorize]
public class EmsController(EmsService emsService, IEesHubNotifier notifier,
    MaintenancePlanService planService) : ControllerBase
{
    ...
    {
        if (!string.IsNullOrEmpty(status) && Enum.TryParse<NexaOne.EMS.Domain.WorkOrderStatus>(status, out var woStatus))
        {
        ...
        [FromQuery] string? status, CancellationToken ct)
    {
        if (!string.IsNullOrEmpty(status) &&
            Enum.TryParse<NexaOne.EMS.Domain.MaintenancePlanStatus>(status, out var s))
        {
