...
import {
  layoutToComponent, componentToLayout, buildDefinitionJson, parseDefinition, flatToLayout,
...
function dto(p: Partial<ScreenDefinitionDto>): ScreenDefinitionDto {
  return {
    uiId: '', title: '', purpose: 'Auto', fields: [], columns: null, queryId: null, saveQueryId: null, layout: null,
    ...p,
  }
}
...
describe('LayoutNode ↔ GrapesJS 매핑', () => {
  ...
  it('aggregate selection/binding scope와 read-only selection 계약을 무손실 보존한다', () => {
    const grid: LayoutNode = {
      kind: 'grid', id: 'lot-grid', queryId: 'POM.LotRoutingContextList',
      selectionScope: 'lot', selectionDisabled: true,
    }
    const form: LayoutNode = { kind: 'form', id: 'lot-form', bindingScope: 'lot', fields: [] }
    const button: LayoutNode = {
      kind: 'commandButton', id: 'track-in', label: 'Track-In',
      command: 'bridge:pom.lot.track-in', bindingScope: 'lot',
    }

    expect(componentToLayout(layoutToComponent(grid))).toEqual(grid)
    expect(componentToLayout(layoutToComponent(form))).toEqual(form)
    expect(componentToLayout(layoutToComponent(button))).toEqual(button)

    const legacyGrid: LayoutNode = { kind: 'grid', id: 'legacy', queryId: 'Q.Legacy' }
    expect(layoutToComponent(legacyGrid).attributes!['data-selection-scope']).toBeUndefined()
    expect(componentToLayout(layoutToComponent(legacyGrid))).toEqual(legacyGrid)
  })
  ...
  it('purpose(화면 목적)가 build/parse 왕복되고 기존 정의는 Auto로 폴백한다', () => {
    const json = buildDefinitionJson('REG1', '등록 화면', null, null, null, null, null, null, 'Register')
    expect(parseDefinition(json).flat!.purpose).toBe('Register')

    const legacy = parseDefinition(JSON.stringify({ uiId: 'OLD', title: '기존', fields: [] }))
    expect(legacy.flat!.purpose).toBe('Auto')
  })
  ...
  it('반복 항목 collection과 하위 field 스키마가 무손실 라운드트립된다', () => {
    const collection: LayoutNode = {
      kind: 'collection', id: 'inspection-items', requiredPermission: 'qms:manage',
      collectionKey: 'items', label: '검사 항목', itemLabel: '항목', bindingScope: 'inspection', minItems: 1, maxItems: 20,
      fields: [
        { kind: 'field', id: 'item-spec', fieldKey: 'specId', field: {
          key: 'specId', label: '검사 규격', type: 'Select', required: true, readOnly: false,
          options: null, optionsQueryId: 'QMS.InspectionSpecCombo',
        } },
      ],
    }

    const component = layoutToComponent(collection)
    expect(component.type).toBe('nx-collection')
    expect(component.attributes!['data-collection-key']).toBe('items')
    expect(component.attributes!['data-binding-scope']).toBe('inspection')
    expect(component.components).toHaveLength(1)
    expect(componentToLayout(component)).toEqual(collection)
  })

  it('잘못된 collection 키와 최소/최대 범위는 DB 저장 JSON 생성 전에 거부된다', () => {
    const invalid: LayoutNode = {
      kind: 'section', children: [{
        kind: 'collection', collectionKey: ' ', label: '항목', itemLabel: '항목', minItems: 2, maxItems: 1,
      }],
    }

    expect(validateLayoutStructure(invalid)).toEqual([
      'layout.children[0].collectionKey가 비어 있습니다.',
      'layout.children[0].maxItems는 minItems 이상의 정수여야 합니다.',
    ])
    expect(() => buildDefinitionJson('BAD', '잘못된 화면', invalid))
      .toThrow(/collectionKey.*maxItems/)
  })
})
...
describe('field 이산 속성 매핑(트레이트 단일 출처)', () => {
  ...
  it('숨김 필드와 UUID 자동 생성 규칙을 Designer 저장 왕복에서 보존한다', () => {
    const node: LayoutNode = {
      kind: 'field', id: 'header-idempotency', fieldKey: 'idempotencyKey',
      field: {
        key: 'idempotencyKey', label: '멱등 키', type: 'Text', required: true, readOnly: false,
        options: null, hidden: true, valueGenerator: 'UuidV4',
      },
    }

    const component = layoutToComponent(node)
    expect(component.attributes!['data-field-hidden']).toBe(true)
    expect(component.attributes!['data-field-value-generator']).toBe('UuidV4')
    expect(componentToLayout(component)).toEqual(node)
  })
...
describe('정의 직렬화', () => {
  it('buildDefinitionJson은 §5 형식(camelCase·평면 필드 빈 배열·layout 포함)을 만든다', () => {
    ...
    expect(parsed.purpose).toBe('Auto')
  ...
  it('평면·bulk 권한 metadata를 저장/로드해도 유실하지 않는다', () => {
    const json = buildDefinitionJson(
      'SECURED', '권한 화면', null,
      null, null, null, 'QMS.Delete',
      [{ label: '승인', commandQueryId: 'QMS.Approve', requiredPermission: 'qms:manage' }],
      'Manage', 'qms:read', 'qms:manage', 'qms:manage',
    )
    const parsed = parseDefinition(json).flat!
    expect(parsed.readRequiredPermission).toBe('qms:read')
    expect(parsed.saveRequiredPermission).toBe('qms:manage')
    expect(parsed.deleteRequiredPermission).toBe('qms:manage')
    expect(parsed.bulkCommands?.[0].requiredPermission).toBe('qms:manage')

    const legacy = parseDefinition(JSON.stringify({ uiId: 'OLD', title: '기존', fields: [] })).flat!
    expect(legacy.readRequiredPermission).toBeNull()
    expect(legacy.saveRequiredPermission).toBeNull()
    expect(legacy.deleteRequiredPermission).toBeNull()
  })
})
...
describe('flatToLayout — 레거시 평면 정의→레이아웃 합성(단방향)', () => {
  ...
  it('평면 정의를 합성할 때 명시 권한과 read/write 카탈로그 권한을 노드에 반영한다', () => {
    const out = flatToLayout(dto({
      uiId: 'PERM', queryId: 'MDM.PlantList', saveQueryId: 'MDM.CreatePlant',
      columns: [{ key: 'ID', caption: 'ID' }],
      fields: [{
        key: 'code', label: '코드', type: 'Select', optionsQueryId: 'SYS.PublicCodes',
      }],
      readRequiredPermission: 'custom:read',
    }), {
      reads: [
        { id: 'MDM.PlantList', isWrite: false, requiredPermission: 'mdm:read' },
        { id: 'SYS.PublicCodes', isWrite: false, requiredPermission: 'sys:read' },
      ],
      writes: [{ id: 'MDM.CreatePlant', isWrite: true, requiredPermission: 'mdm:manage' }],
    }) as Section

    const row = out.children![0] as Row
    const grid = (row.children![0] as Column).children![0]
    const formColumn = row.children![1] as Column
    const form = formColumn.children![0]
    const button = formColumn.children![1]
    expect(grid).toMatchObject({ kind: 'grid', requiredPermission: 'custom:read' })
    expect(form).toMatchObject({ kind: 'form', requiredPermission: 'mdm:manage' })
    expect(button).toMatchObject({ kind: 'commandButton', requiredPermission: 'mdm:manage' })
    expect(form.kind === 'form' && form.fields?.[0].requiredPermission).toBe('sys:read')
  })
})
