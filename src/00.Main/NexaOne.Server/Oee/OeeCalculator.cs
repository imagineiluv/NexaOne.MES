...
/// 계획시간 = 스케줄 상태(가동+비가동) 합, 가용성 = 가동/계획, 성능 = (이상사이클×총생산)/(가동초),
/// 품질 = 양품/총생산. 비율은 [0,1]로 클램프(데이터 이상 시 OEE≤1 보장).</summary>
public static class OeeCalculator
{
        ...
        OeeStateCategory unknownCategory)
    {
        ...
        // 상태이력이 없거나 스케줄 시간이 0이면 목표 계획시간으로 폴백(가용성 산출 불가 방지).
        if (planned <= 0m) planned = target.PlannedMinutes;
