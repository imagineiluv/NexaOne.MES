...
public sealed class SysRefreshTokenStore : IRefreshTokenStore
{
    ...
    public async Task<string> IssueAsync(string userId)
    {
        ...
        await _dispatcher.ExecuteAsync(_authQueries.Sql("SYS.InsertRefreshToken"), new Dictionary<string, object>
        {
    ...
    {
        var rows = await _dispatcher.QueryAsync(_authQueries.Sql("SYS.ValidateRefreshToken"), new Dictionary<string, object>
        {
            ["userId"] = userId,
            ["tokenHash"] = Hash(token),
    ...
    public async Task<string> RotateAsync(string userId, string oldToken)
    {
        ...
        var affected = await _dispatcher.ExecuteAsync(_authQueries.Sql("SYS.RevokeRefreshTokenIfActive"), new Dictionary<string, object>
        {
            ["userId"] = userId,
    ...
    public async Task RevokeAsync(string userId, string token)
        => await _dispatcher.ExecuteAsync(_authQueries.Sql("SYS.RevokeRefreshTokenIfActive"), new Dictionary<string, object>
        {
            ["userId"] = userId,
            ["tokenHash"] = Hash(token),
            ["utcNow"] = DateTime.UtcNow,
    ...
    public async Task RevokeAllByUserAsync(string userId)
        => await _dispatcher.ExecuteAsync(_authQueries.Sql("SYS.RevokeAllRefreshTokens"), new Dictionary<string, object>
        {
            ["userId"] = userId,
            ["utcNow"] = DateTime.UtcNow,
        });
