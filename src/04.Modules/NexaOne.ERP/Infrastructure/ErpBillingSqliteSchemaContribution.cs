...
public sealed class ErpBillingSqliteSchemaContribution : ISqliteSchemaContribution
{
    ...
    public void Apply(DbConnection connection, DbTransaction transaction)
    {
        ...
        EnsureExpenseTagResources(connection, transaction);
    ...
    private static void EnsureExpenseTagResources(DbConnection connection, DbTransaction transaction)
    {
        if (!HasTable(connection, transaction, "SYS_MULTI_LANGUAGE_RESOURCE")) return;
        Execute(connection, transaction, """
            ...
            WHERE NOT EXISTS (
                SELECT 1 FROM SYS_MULTI_LANGUAGE_RESOURCE existing
