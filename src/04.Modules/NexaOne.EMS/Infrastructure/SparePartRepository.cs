...
public sealed class SparePartRepository : QueryRepository, ISparePartRepository
{
    ...
    {
        const string sql = "SELECT * FROM EMS_SPARE_PART WITH(NOLOCK) WHERE PART_ID = @partId";
    ...
    {
        const string sql = "SELECT * FROM EMS_SPARE_PART WITH(NOLOCK) ORDER BY PART_NAME";
    ...
    {
        const string sql = "SELECT * FROM EMS_SPARE_PART WITH(NOLOCK) WHERE CURRENT_STOCK <= MIN_STOCK ORDER BY PART_NAME";
        var rows = await QueryAsync<PartRow>(sql, new { }, ct);
        return rows.Select(r => r.ToDomain()).OfType<SparePart>().ToList();
    }
