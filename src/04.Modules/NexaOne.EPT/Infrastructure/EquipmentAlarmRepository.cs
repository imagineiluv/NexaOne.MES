...
namespace NexaOne.EPT.Infrastructure;
...
public sealed class EquipmentAlarmRepository : QueryRepository, IEquipmentAlarmRepository
{
    ...
    {
        const string sql = "SELECT * FROM EPT_EQUIPMENT_ALARM WITH(NOLOCK) WHERE ALARM_ID = @alarmId";
    ...
    {
        const string sql = @"SELECT * FROM EPT_EQUIPMENT_ALARM WITH(NOLOCK)
    ...
    {
        const string sql = @"SELECT a.* FROM EPT_EQUIPMENT_ALARM a WITH(NOLOCK)
    ...
    {
        const string sql = "SELECT COUNT(*) FROM EPT_EQUIPMENT_ALARM WITH(NOLOCK) WHERE CLEARED_AT IS NULL";
        return await CountAsync(sql, null, ct);
    }

    public async Task AddAsync(EquipmentAlarm alarm, CancellationToken ct = default)
    {
        const string sql = @"INSERT INTO EPT_EQUIPMENT_ALARM
    ...
    {
        const string sql = @"UPDATE EPT_EQUIPMENT_ALARM SET
