...
public sealed class DeliveryItemRepository : QueryRepository, IDeliveryItemRepository
{
    ...
    {
        const string sql = "SELECT * FROM DLV_DELIVERY_ITEM WITH(NOLOCK) WHERE ITEM_ID = @itemId";
    ...
    {
        const string sql = "SELECT * FROM DLV_DELIVERY_ITEM WITH(NOLOCK) WHERE DELIVERY_ORDER_ID = @orderId ORDER BY CREATED_AT";
    ...
    {
        const string sql = @"INSERT INTO DLV_DELIVERY_ITEM
    ...
    {
        const string sql = @"UPDATE DLV_DELIVERY_ITEM SET
