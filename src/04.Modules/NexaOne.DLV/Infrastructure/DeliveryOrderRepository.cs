...
public sealed class DeliveryOrderRepository : QueryRepository, IDeliveryOrderRepository
{
    ...
    {
        const string sql = "SELECT * FROM DLV_DELIVERY_ORDER WITH(NOLOCK) WHERE ORDER_ID = @orderId";
    ...
    {
        const string sql = @"SELECT * FROM DLV_DELIVERY_ORDER WITH(NOLOCK)
    ...
    {
        const string sql = "SELECT COUNT(*) FROM DLV_DELIVERY_ORDER WITH(NOLOCK) WHERE STATUS = @status";
        return await CountAsync(sql, new { status }, ct);
    }

    public async Task AddAsync(DeliveryOrder order, CancellationToken ct = default)
    {
        const string sql = @"INSERT INTO DLV_DELIVERY_ORDER
    ...
    {
        const string sql = @"UPDATE DLV_DELIVERY_ORDER SET
