...
public sealed class ProductionOrderRepository : QueryRepository, IProductionOrderRepository
{
    ...
    {
        const string sql = "SELECT * FROM PPM_PRODUCTION_ORDER WITH(NOLOCK) WHERE ORDER_ID = @orderId";
    ...
    {
        const string sql = "SELECT * FROM PPM_PRODUCTION_ORDER WITH(NOLOCK) WHERE PLAN_ID = @planId ORDER BY SCHEDULED_START";
    ...
    {
        const string sql = @"INSERT INTO PPM_PRODUCTION_ORDER
    ...
    {
        const string sql = @"UPDATE PPM_PRODUCTION_ORDER SET
