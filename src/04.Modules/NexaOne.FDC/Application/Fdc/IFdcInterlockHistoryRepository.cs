...
{
    Task<FdcInterlockHistory?> GetByIdAsync(string effectId, CancellationToken ct = default);
    ...
    Task<IReadOnlyList<FdcInterlockHistory>> GetAllUnresolvedAsync(CancellationToken ct = default);
    Task<IReadOnlyList<FdcInterlockHistory>> GetUnresolvedAsync(string equipmentId, CancellationToken ct = default);
    Task<IReadOnlyList<FdcInterlockHistory>> GetUnresolvedAsync(
        string equipmentId,
        string parameterId,
        CancellationToken ct = default);
    Task AddAsync(FdcInterlockHistory history, CancellationToken ct = default);
    Task<bool> UpdateAsync(FdcInterlockHistory history, int expectedVersion, CancellationToken ct = default);
